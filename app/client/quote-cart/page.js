...
export default function QuoteCartPage() {
  ...
  const validateClient = () => {
    ...
    if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i.test(client.email.trim())) {
  ...
  const updateQty = async (productId, action) => {
    ...
    if (action === "inc" && item.qty >= item.stock_qty) {
      showToast(`Only ${item.stock_qty} items available in stock`, "warning");
      return;
    }
    ...
    return (
      <div className={styles.centerWrapper}>
        <div className={styles.successCard}>
          ...
          <h4 className={styles.successTitle}>RFQ Submitted Successfully</h4>
              ...
              <div className={styles.cartBox}>
                <div className={styles.tableResponsive}>
                  ...
                  <div className={styles.tableNote}>
                    *Indicative amount only. Final quotation will be shared
                    after RFQ review.
                  </div>
                </div>
              </div>
